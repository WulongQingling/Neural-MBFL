...
def init(handler):
  ...
  aggregation_list = ["MaxSubAvg","Max"]
        ...
        for type in type_list: # 每个type
          for Tie_break in Tie_breakList: # 每个tie_break
            ...
            for approach in approachList: # 每个项目
              print(f"{filename} {item} {aggregation} {type} {Tie_break}")
              # MaxSubAvgMajorType1RankAve 
              faulty_line_rank_path = f"/home/{aggregation}{filename}{type}Rank{Tie_break}/{approach}/{item}.json" # 这个文件那里来的
              ...
              dict_tmp = {
                ...
                "FileName": f"{aggregation}{filename}{type}Rank{Tie_break}"
              }
...
if __name__ == "__main__":
    ...
    init(handler)
    handler.save_data("/home/d4jscript/hblscript/MAP/hblclacMAPMetallaxis.xlsx")  # Specify your filename