...
def init(pro, svid, evid): 
  ...
  # filename = ['susMaxMajorType3','susMaxMbertType3']
  filename = ['MusesusMaxMbertType3','MusesusMaxMajorType3']
  # filename = ['susMaxMajorMbertSFCluFOMType3']
  ...
  mutant_path = "/home/faultyLine/{}FalutLine.txt".format(pro)# 语句级错误行的路径
  # mutant_path = "/home/faultyFunc/{}FalutLine_function.txt".format(pro)# 方法错误行的路径
  ...
  for item in range(svid, evid + 1):
    ...
    for formula_item in formula:
      for _item in filename:
        mbert_sus_path = f"/home/{_item}/{pro}/{item}b/{formula_item}.csv" # 每个版本的怀疑度csv文件的路径,我的文件忘记加b了，记得加
        ...
        mbert_rank_path = f"/home/{tmp[_item]}/{pro}/{item}b" # 生成rank值的目标路径
